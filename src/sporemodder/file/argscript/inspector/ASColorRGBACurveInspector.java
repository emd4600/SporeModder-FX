...
public class ASColorRGBACurveInspector extends ASValueInspector {
	...
	private final List<ColorRGBA> colors = new ArrayList<ColorRGBA>();
	 ...
	 */
	public ASColorRGBACurveInspector(String name, String descriptionCode, ColorRGBA defaultValue, boolean requiresValue) {
		this(name, descriptionCode, false, defaultValue, requiresValue);
	}
	
	public ASColorRGBACurveInspector(String name, String descriptionCode, boolean is255, ColorRGBA defaultValue, boolean requiresValue) {
	...
	private void insertElements() {
		...
		for (int i = 0; i < count; i++) {
			...
			if (is255) lineInspector.getStream().parseColorRGBA255(args, argIndex + i, value);
			else lineInspector.getStream().parseColorRGBA(args, argIndex + i, value);
			colors.add(value);
		}
		...
		for (int i = 0; i < count; i++) {
			...
			colorPicker.setOnAction((event) -> {
				...
				colors.set(index, new ColorRGBA(colorPicker.getValue()));
				updateGradient();
