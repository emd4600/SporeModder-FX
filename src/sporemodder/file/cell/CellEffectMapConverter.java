	...
	public void generateContextMenu(ContextMenu contextMenu, ProjectItem item) {
		if (!item.isRoot()) {
			...
			if (item.isMod() && isEncoder(item.getFile())) {
				MenuItem menuItem = new MenuItem("Convert to EFFECTMAP");
				...
				if (isDecoder(key)) {
					MenuItem menuItem = new MenuItem("Convert to EFFECTMAP_T");
					menuItem.setMnemonicParsing(false);
					menuItem.setOnAction(event -> {
						final File outputFile = Converter.getOutputFile(key, item.getFile().getParentFile(), "effectMap_t");
						boolean result = UIManager.get().tryAction(() -> {
