	...
	public void generateContextMenu(ContextMenu contextMenu, ProjectItem item) {
		if (!item.isRoot()) {
			...
			if (item.isMod() && isEncoder(item.getFile())) {
				MenuItem menuItem = new MenuItem("Convert to GLOBALS");
				...
				if (isDecoder(key)) {
					MenuItem menuItem = new MenuItem("Convert to GLOBALS_T");
					menuItem.setMnemonicParsing(false);
					menuItem.setOnAction(event -> {
						final File outputFile = Converter.getOutputFile(key, item.getFile().getParentFile(), "globals_t");
						boolean result = UIManager.get().tryAction(() -> {
