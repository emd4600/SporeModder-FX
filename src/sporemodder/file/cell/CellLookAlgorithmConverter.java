	...
	public void generateContextMenu(ContextMenu contextMenu, ProjectItem item) {
		if (!item.isRoot()) {
			...
			if (item.isMod() && isEncoder(item.getFile())) {
				MenuItem menuItem = new MenuItem("Convert to LOOK_ALGORITHM");
				...
				if (isDecoder(key)) {
					MenuItem menuItem = new MenuItem("Convert to LOOK_ALGORITHM_T");
					menuItem.setMnemonicParsing(false);
					menuItem.setOnAction(event -> {
						final File outputFile = Converter.getOutputFile(key, item.getFile().getParentFile(), "look_algorithm_t");
						boolean result = UIManager.get().tryAction(() -> {
