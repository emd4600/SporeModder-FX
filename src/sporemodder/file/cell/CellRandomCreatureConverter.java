	...
	public void generateContextMenu(ContextMenu contextMenu, ProjectItem item) {
		if (!item.isRoot()) {
			...
			if (item.isMod() && isEncoder(item.getFile())) {
				MenuItem menuItem = new MenuItem("Convert to RANDOM_CREATURE");
				...
				if (isDecoder(key)) {
					MenuItem menuItem = new MenuItem("Convert to RANDOM_CREATURE_T");
					menuItem.setMnemonicParsing(false);
					menuItem.setOnAction(event -> {
						final File outputFile = Converter.getOutputFile(key, item.getFile().getParentFile(), "random_creature_t");
						boolean result = UIManager.get().tryAction(() -> {
