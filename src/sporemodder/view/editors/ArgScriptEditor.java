...
public abstract class ArgScriptEditor<T> extends TextEditorWithErrors {
	...
	protected void hyperlinkOpenFile(String[] names) {
		...
		// By default, try .prop/.audioProp/.submix files
		...
		if (names[2] != null) {
			...
			if (names[2].equals(prop) || names[2].equals(audioProp) || names[2].equals(soundProp) || names[2].equals(submix)) {
				path = path + ".prop_t";
			}
		} else {
			File file = getFile();
			...
			if (file.getName().contains(audioProp)) {
				...
				path = path + '.' + soundProp;
			} else if (file.getName().contains(submix)) {
				path = path + '.' + submix;
			} else {
				path = path + '.' + prop;
			}
