...
import sporemodder.file.gait.GaitFile;
import sporemodder.view.UserInterface;

public class GaitEditor extends ArgScriptEditor<GaitFile> {
	
	public GaitEditor() {
		super();
		
		GaitFile unit = new GaitFile();
	...
	@Override protected void showInspector(boolean show) {
		if (show) {
			UserInterface.get().getInspectorPane().configureDefault("Gait Definition (GAIT)", "GAIT", null);
		} else {
